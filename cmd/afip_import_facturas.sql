...
DELETE import_cabecera
WHERE CAI IN (SELECT CAI FROM CABS_FACTURAS);

DELETE IMPORT_DETALLE
WHERE ROWID NOT IN (SELECT D.ROWID 
FROM IMPORT_DETALLE D,import_cabecera F
	WHERE D.TIPO_COMPROBANTE = F.TIPO_COMPROBANTE
	AND D.PUNTO_VENTA = F.PUNTO_VENTA
	AND D.NRO_COMPROBANTE = F.NRO_COMPROBANTE);
...
SELECT
	...
	CAI,
	CASE WHEN TIPO_COMPROBANTE IN (3,8,20) THEN -1 ELSE 1 END 
...
--TRUNCATE TABLE IMPORT_CABECERA;
/

--TRUNCATE TABLE IMPORT_DETALLE;
/
